...
# Bob's MCP Server
# Provides repository operations tools via Cloud API Registry
# Registered in API Registry for runtime tool discovery by agents
resource "google_cloud_run_service" "bobs_mcp" {
  ...
  template {
    spec {
      ...
      containers {
        ...
        # Resource limits (MCP servers need modest resources)
        resources {
          limits = {
            cpu    = "1000m"
            memory = "512Mi"
  ...
  depends_on = [
    google_service_account.bobs_mcp,
    google_project_iam_member.bobs_mcp_logging,
  ]
}

# Bob's MCP Server IAM Policy
# MCP servers should only be accessible by authenticated agents
# NOT public access - agents must authenticate via API Registry
resource "google_cloud_run_service_iam_member" "bobs_mcp_agents" {
  count = var.bobs_mcp_enabled ? 1 : 0

  service  = google_cloud_run_service.bobs_mcp[0].name
  location = google_cloud_run_service.bobs_mcp[0].location
  role     = "roles/run.invoker"
  # Allow Agent Engine service account to invoke MCP server
