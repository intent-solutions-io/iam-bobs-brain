...
class ContentExtractor:
    ...
    def extract_code_blocks(self, soup: BeautifulSoup) -> List[Dict]:
        ...
        for pre in soup.find_all("pre"):
            ...
            if code_text.strip():
                code_blocks.append(
                    {"language": language or "text", "code": code_text.strip()}
                )
    ...
    def extract_content(
        self, html: str, url: str, doc_id: str, title: str, timestamp: str
    ) -> Dict:
        """
...
def extract_all_documents(crawled_pages: List[Dict]) -> List[Dict]:
    ...
    logger.info(
        f"Successfully extracted {len(documents)}/{len(crawled_pages)} documents"
    )
    return documents
