...
class SlackGatewayConfigValidator:
    ...
    def check_labels(self):
        ...
        has_labels = any(
            "labels" in line.lower() for line in open(self.config_file).readlines()
        )
    ...
    def validate(self) -> Tuple[bool, List[ValidationError]]:
        ...
        print(
            f"{Colors.BLUE}Validating Slack gateway configuration for {self.env}...{Colors.END}"
        )
        print()
...
def main():
    ...
    if env not in SlackGatewayConfigValidator.VALID_ENVIRONMENTS:
        ...
        print(
            f'Valid environments: {", ".join(SlackGatewayConfigValidator.VALID_ENVIRONMENTS)}'
        )
        return 2
